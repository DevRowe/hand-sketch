...
- `src/explorer/`: the live Solar System Explorer (entry `explorer/index.html`, config `vite.explorer.config.ts`, build committed under `docs/explorer/`, never edited by hand).
  ...
  To measure it, run `node scripts/explorer-bench.mjs --profile desktop|phone` against a served build (`npx vite preview --outDir docs --port 5199`): every style, view and mode (play, trails off, zoom, jump) timed per drawing with the raster flushed, `--cpuprofile style:view:mode` for hot functions (build with `--minify false` to read them); it skips the draw-on, whose frames say nothing of the steady cost; by hand, time `app.renderer.draw` plus a `getImageData(0,0,1,1)` raster flush; CDP `Emulation.setCPUThrottlingRate` 4 approximates a laptop or phone, and a phone's DPR 3 needs Chrome launched with `--force-device-scale-factor=3` (an emulated DPR does not reach the canvas's backing).
  Frame cost here is mostly raster, and a scaled `drawImage` costs 3 to 5 times a 1:1 one, so: marks that stand still once drawn go in `held` (or a wholly still plate takes `ink`'s `fixed`; `gallery/common`), which a render draws straight through and the explorer at rest keeps as a page layer (context state a held block leaves behind must be set again after it); a view on the move lays page layers unsmoothed (`Stage.coarse`, textures always); the wakes are sampled coarser where a design unit is under a device pixel, and coarser yet while the view moves on a slow device (`TrailSpec.spacing`, `app.wakeSpacing`); camera moves draw only as often as leaves a third of the time free for input (`renderer.cost`).
  Batching many small fills into one path is not a win in software raster (measured: hundreds of `fillRect`s beat one path of rects), and removing even fully clipped subpaths from a render's path changes its bytes, so gate such culls to the live sky (`sky.trails`).
