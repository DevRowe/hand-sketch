#!/usr/bin/env node
// The solar set ("One Sky, Ten Hands"): web delivery for the ten top-down solar systems plus a board that sets them
// side by side.
//
// For every entry of src/scenes/solar/catalog.json this runs `scripts/render.mjs --web` (12 fps H.264 + VP9 with a
...
//
// Usage (after `npm run build`, or via `npm run solar`):
//   node scripts/solar.mjs                 render all ten into output/solar/ and write the board
//   node scripts/solar.mjs --only S01,S07  re-render some, rewrite the board from every sidecar present
//   node scripts/solar.mjs --verify        also render each twice in independent page loads and fail on any difference
//   node scripts/solar.mjs --out dir       output directory (default output/solar/)
//   node scripts/solar.mjs --board-only    just rewrite board.html and manifest.json from existing sidecars
import { writeFileSync } from 'node:fs';
import path from 'node:path';
import { BOARD_SCRIPT, esc, loadEntries, readCatalog, renderSet, setOptions } from './lib/web-set.mjs';

const opts = setOptions(process.argv.slice(2), 'output/solar');
const { outDir } = opts;
const catalog = readCatalog('src/scenes/solar/catalog.json');
...
<head>
  ...
  <title>One Sky, Ten Hands</title>
  <meta name="description" content="The solar system seen from above, drawn in code ten times in ten visual languages.">
...
<body>
  <header>
    <p class="kicker">hand-sketch &middot; solar</p>
    <h1>One Sky, Ten Hands</h1>
    <p>The solar system from straight above: the Sun at the centre, eight planets on their orbits, the Moon round the Earth, the belt between Mars and Jupiter. The same plan and the same clock, drawn ten times in ten visual languages. Every planet makes a whole number of orbits in one loop, twelve for Mercury down to one for Neptune, so each piece closes seamlessly. All of it is drawn in code on one engine, deterministic to the pixel. The first plays on its own; tap any other to play it.</p>
  </header>
  <main>${figure(hero, true)}
    <section aria-labelledby="ten-title">
      <h2 id="ten-title">The ten</h2>
      <p>Side by side, each resting on the same moment of the same loop.</p>
