...
function buildPaper(stage: Stage, g: Ctx, o: Required<Pick<PaperOptions, 'color' | 'seed' | 'texture'>> & PaperOptions): void {
  const { w: W, h: H } = stage, dark = isDark(o.color), r = rng(o.seed);
  ...
  if (m) {
    ...
    for (let y = 0; y < mh; y++) {
      for (let x = 0; x < mw; x++) {
        const n = noise2(x / 7, y / 7, o.seed) + 0.5 * noise2(x / 2.5, y / 2.5, o.seed + 9);
        const k = (y * mw + x) * 4;
        img.data[k] = cr; img.data[k + 1] = cg; img.data[k + 2] = cb;
        img.data[k + 3] = Math.max(0, Math.min(255, (n + 0.35) * 70 * o.texture));
      }
    }
  ...
  g.save();
  g.fillStyle = speckle;
  g.globalAlpha = (dark ? 0.35 : 0.08) * o.texture;
  g.beginPath();
